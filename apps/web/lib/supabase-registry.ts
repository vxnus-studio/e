...
export async function publishPack(input: { projectId: string; ownerId: string; pack: RegistryPack; revisionManifest: unknown; revisionId: string }) {
  ...
  const owned = await database.select({ id: publisherProjects.id, publisher: publisherProjects.publisher }).from(publisherProjects).where(and(eq(publisherProjects.id, input.projectId), eq(publisherProjects.ownerId, input.ownerId))).limit(1);
  if (!owned[0]) throw new Error("You do not own this project.");
  const identity = input.pack.id.match(/^@([^/]+)\/([^/]+)$/);
  if (!identity || identity[1] !== owned[0].publisher || input.pack.publisher !== owned[0].publisher) throw new Error("The provider identity must match the selected project publisher.");
