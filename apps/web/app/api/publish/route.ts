...
export async function POST(request: Request) {
  ...
  if (kind === "url") {
    ...
    try {
      ...
      const pack = {
        ...
        verified: true,
        distributionType: "remote" as const,
        distribution: { kind: "provider" as const, url: canonicalProviderUrl },
        distributions: [{ kind: "provider" as const, url: canonicalProviderUrl }],
        apiContract,
      };
  ...
  try {
    ...
    await publishPack({
      ...
      revisionId: pack.revision.id,
      revisionManifest: pack.manifest,
      pack: {
        id: pack.manifest.id,
        ...
        publisherId: session.user.id,
        verified: false,
        distributionType: "local" as const,
        distribution: { kind: "archive", url: r2.publicUrl(key), checksum },
        distributions: [{ kind: "archive", url: r2.publicUrl(key), checksum }],
      },
    });
