...
async function PublicProjectPage({ params }: { params: Promise<{ publisher: string; project?: string }> }) {
  ...
  return <main className="detail-page"><CatalogRefresh />
    <nav className="site-nav" aria-label="Primary navigation"><a className="brand" href="/"><span className="brand-mark">E</span> Hub</a><div className="nav-links"><a href="/catalog">Catalog</a><a href="/publish">Publish</a><a className="nav-button" href="/auth/sign-in">Sign in</a></div></nav>
    <section className="detail-hero" aria-labelledby="project-title"><a className="back-link" href="/catalog">← Back to catalog</a><div className="detail-heading"><div><p className="eyebrow">Public project</p><h1 id="project-title">{projectName}</h1><p className="detail-package">@{namespace}</p><p className="detail-lede">{project?.description || "A public capability project published through E Hub."}</p></div><div className="detail-version"><span>Project status</span><strong>{project?.visibility || "public"}</strong><small>Publisher workspace</small></div></div></section>
