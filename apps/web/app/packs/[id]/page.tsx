...
export function generateStaticParams() { return [{ id: pack.id }]; }

export default async function PackPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  if (id !== pack.id) notFound();
  ...
  return <main className="detail-page">
    ...
    <section className="detail-hero" aria-labelledby="pack-title"><a className="back-link" href="/#catalog">← Back to catalog</a><div className="detail-heading"><div><p className="eyebrow">Knowledge pack · verified</p><h1 id="pack-title">{pack.name}</h1><p className="detail-lede">{pack.description}</p></div><div className="detail-version"><span>Current release</span><strong>v{pack.version}</strong><small>Revision {pack.revision}</small></div></div></section>
